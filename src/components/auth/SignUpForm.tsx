...
// Sign-up form body — rendered inside the homepage AuthModal. Name / email /
// password / confirm + the 6-digit email-verification step. Headless Clerk v7
// flow; zero Clerk UI. Stashes ?plan=<slug> for the dashboard checkout resume.
...
import { Field, inputCls } from '@/components/auth/AuthShell';
...
export default function SignUpForm({ onSwitch, onDone }: { onSwitch: () => void; onDone: () => void }) {
  ...
  // A pricing CTA may open the modal with ?plan=pro — stash it so UpgradeCta
  // on the dashboard resumes checkout right after the account is created.
  ...
  if (verifying) {
    return (
      <>
        <h2 className="text-2xl font-bold text-white tracking-tight">{en ? 'Check your email' : 'შეამოწმე ელფოსტა'}</h2>
        <p className="text-sm text-gray-400 mt-1.5 mb-6 leading-relaxed">
          {en
  ...
  return (
    <>
      <h2 className="text-2xl font-bold text-white tracking-tight">{en ? 'Create your account' : 'შექმენი ანგარიში'}</h2>
      <p className="text-sm text-gray-400 mt-1.5 mb-6 leading-relaxed">{en ? '7 days free. No credit card required.' : '7 დღე უფასოდ. საკრედიტო ბარათი არ გჭირდება.'}</p>
      ...
      <p className="text-center mt-5 text-sm text-gray-400">
        ...
        <button type="button" onClick={onSwitch} className="text-blue-400 hover:text-blue-300 font-semibold">
          {en ? 'Sign in' : 'შესვლა'}
        </button>
      </p>
    </>
