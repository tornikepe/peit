...
export default function BackupLogView() {
  ...
  if (state === 'forbidden') return (
    <div className="rounded-2xl border border-white/[0.08] bg-[#0d0d1a] p-8 text-center">
      ...
      <p className="text-sm text-gray-400">ეს გვერდი მხოლოდ ადმინისტრატორებისთვისაა.</p>
    </div>
  );
  if (state === 'error') return <p className="text-sm text-gray-500">ჩატვირთვა ვერ მოხერხდა.</p>;

  return (
    <div className="rounded-2xl border border-white/[0.06] bg-[#0d0d1a] overflow-hidden">
      <div className="px-5 py-3 border-b border-white/[0.06] flex items-center gap-2">
        <Database className="w-4 h-4 text-blue-300" />
        <h3 className="text-sm font-semibold text-white">ბოლო backup-ები</h3>
        <span className="text-xs text-gray-500">(Neon managed + PITR)</span>
      </div>
      {rows && rows.length === 0 ? (
        <div className="px-5 py-10 text-center text-sm text-gray-500">ჯერ ჩანაწერი არ არის — cron ღამის 02:00-ზე გაეშვება.</div>
      ) : (
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-xs text-gray-500">
              <th className="px-5 py-2 font-medium">სტატუსი</th>
              <th className="px-5 py-2 font-medium">ზომა</th>
              <th className="px-5 py-2 font-medium">ხანგრძლივობა</th>
              <th className="px-5 py-2 font-medium text-right">თარიღი</th>
            </tr>
          </thead>
          <tbody>
            {rows?.map(r => (
              <tr key={r.id} className="border-t border-white/[0.05]">
                <td className="px-5 py-3">
                  {r.status === 'success'
                    ? <span className="inline-flex items-center gap-1.5 text-emerald-300"><CheckCircle2 className="w-3.5 h-3.5" /> წარმატება</span>
                    : <span className="inline-flex items-center gap-1.5 text-red-300" title={r.error ?? ''}><XCircle className="w-3.5 h-3.5" /> შეცდომა</span>}
                </td>
                ...
                <td className="px-5 py-3 text-right text-gray-400">{new Date(r.createdAt).toLocaleString('ka-GE')}</td>
              </tr>
            ))}
          </tbody>
        </table>
